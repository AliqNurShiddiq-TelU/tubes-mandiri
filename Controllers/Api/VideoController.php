...
class VideoController extends Controller
{
    ...
    public function update(Request $request, $id_video)
    {
        ...
        $request->validate([
            'url_video' => 'nullable|url', // Link Youtube baru
            'durasi'    => 'nullable|string'
        ]);

        $dataUpdate = [];

        // Jika user kirim link baru
        if ($request->filled('url_video')) {
            $dataUpdate['file_path'] = $this->convertToEmbedUrl($request->url_video);
        }

        // Jika user kirim durasi baru
